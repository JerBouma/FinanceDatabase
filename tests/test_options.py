...
def test_different_options():
    ...
    assert (
        type(cryptocurrencies) is list and len(cryptocurrencies) > 0
    ), "cryptocurrencies is incorrect"
    ...
    assert (
        type(equities) is list and len(equities) > 0
    ), "equities countries is incorrect"
    ...
    assert (
        type(equities) is list and len(equities) > 0
    ), "equities industries is incorrect"

    equities = show_options("equities", sector="Technology")
    assert (
        type(equities) is dict and len(equities) > 0
    ), "equities industries is incorrect"
    ...
    equities = show_options(
        "equities", country="United States", sector="Technology", industry=True
    )
    assert (
        type(equities) is dict and len(equities) > 0
    ), "equities industries is incorrect"
