...
stock_data_biotech = yf.download(
    list(health_etfs_in_biotech.keys()), start="2020-01-01", end="2020-06-01"
)["Adj Close"]
stock_data_biotech = stock_data_biotech.dropna(axis="columns")

# Initialise the plot and row/column variables
...
for ticker in stock_data_biotech.columns:
    ...
    indicator_bb = BollingerBands(
        close=stock_data_biotech[ticker], window=20, window_dev=2
    )
