...
def fill_data_points_futures(data_symbol, options=None):
    if options is None:
        options = {}
    try:
...
def make_directories_and_fill_json_futures(data, directory_name):
    try:
    ...
    for symbol in tqdm(data):
        options = fill_data_points_futures(data[symbol])
