...
XLSX_NAME_OF_ALL_SYMBOLS = "Ticker Dataset Filtered.xlsx"
THIS_FOLDER = os.path.dirname(os.path.abspath(__file__))
my_file = os.path.join(THIS_FOLDER, XLSX_NAME_OF_ALL_SYMBOLS)
ticker_list = pd.read_excel(my_file, index_col=[0])
        ...
        for option in tqdm(options):
            ...
            for ticker in tickers:
                try:
                    ...
                    print(f"{Fore.RED} Could not find this pickle: {error}")
...
def collect_data(ticker):
    ...
    try:
        ...
        retries = 0
        symbol_type = ticker_list.loc[ticker]['Type Display']
...
if __name__ == "__main__":
    ...
    ticker_symbols = list(ticker_list.index)
