...
def test_random_options():
    ...
    assert (
        type(random_crypto) is dict and len(random_crypto) > 0
    ...
    assert (
        type(random_currency) is dict and len(random_currency) > 0
    ), "random_currency dictionary is invalid."

    etfs_option = show_options("etfs")[random_number]
    random_etf = select_etfs(etfs_option)
    assert (
        type(random_etf) is dict and len(random_etf) > 0
    ), "random_etf dictionary is invalid."
    ...
    assert (
        type(random_equity) is dict and len(random_equity) > 0
    ), "random_equity (countries) dictionary is invalid."
    ...
    assert (
        type(random_equity) is dict and len(random_equity) > 0
    ), "random_equity (sectors) dictionary is invalid."

    equity_option_industries = show_options("equities")["industries"][random_number]
    random_equity = select_equities(
        industry=equity_option_industries, exclude_exchanges=False
    )
    assert (
        type(random_equity) is dict and len(random_equity) > 0
    ), "random_equity (industries) dictionary is invalid."
    ...
    assert type(random_equity) is dict and len(random_equity) > 0, (
        "random_equity (country + sector) " "dictionary is invalid."
    )

    random_equity = select_equities(
        country="United States", industry="Waste Management"
    )
    assert type(random_equity) is dict and len(random_equity) > 0, (
        "random_equity (country + industry) " "dictionary is invalid."
    )

    random_equity = select_equities(
        country="United States", sector="Consumer Cyclical", industry="Auto Parts"
    )
    assert type(random_equity) is dict and len(random_equity) > 0, (
        "random_equity (country + sector + industry) " "dictionary is invalid."
    )

    funds_option = show_options("funds")[random_number]
    random_fund = select_funds(funds_option, exclude_exchanges=False)
    assert (
        type(random_fund) is dict and len(random_fund) > 0
    ), "random_fund dictionary is invalid."

    random_index = select_indices("us_market")
    assert (
        type(random_index) is dict and len(random_index) > 0
    ), "random_index dictionary is invalid."

    random_moneymarket = select_moneymarkets("us_market")
    assert (
        type(random_moneymarket) is dict and len(random_moneymarket) > 0
    ), "random_moneymarket dictionary is invalid."
